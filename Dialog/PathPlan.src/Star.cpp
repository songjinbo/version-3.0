...
bool Star::Creatgraph()
{
	...
	{
		if (get_voxel_status == get_all_voxel_complete)
		{
			path_plan_status = path_accessible;			//最终找到可以利用的路径
			return 0;
		}

		path_plan_status = no_voxel_in_queue;
		return 0;
	}
	...
	FILE *fp; 
	if ((fp = fopen(outputfile1_3DAStar.c_str(), "a")) != 0) //全局变量3
	{
		...
		for (i = 0 ; i < zDepth ; i++ )
		{
			for (j  = 0; j < yDepth ; j++ )
			{
				for (k =0 ; k < xDepth ; k++ )
				{
					...
					if (isWalkable == false) 
					{
						fprintf(fp, "%5d ", pi.xPos);
						fprintf(fp, "%5d ", pi.yPos);
						fprintf(fp, "%5d ", pi.zPos);
						fprintf(fp, "%5d \n", 0); 
					}
	...
	if (is_first_frame)
	{
		is_first_frame = 0; //置标志位，表明已经不是第一帧
	...
	double mediump[3];
	if (minValue[0] <= (farthp[0] - flightsize) && (farthp[0] - flightsize) <= maxValue[0])
	{
	...
	if (minValue[1] <= (farthp[1] - flightsize) && (farthp[1] - flightsize) <= maxValue[1])
	{
	...
	int mediup[3];
	for (int i = 0; i<3; i++)
		if (minValue[i] <= mediump[i] - flightsize && mediump[i] - flightsize <= maxValue[i] && minValue[i] <= mediump[i] + flightsize && mediump[i] + flightsize <= maxValue[i])
		{
			//endRealPosition[i]=mediump[i];
			mediup[i] = int((mediump[i] - minValue[i]) / sizeofBlock + 0.5);
		}

	if (minValue[0] <= mediump[0] && mediump[0] <= maxValue[0] && minValue[1] <= mediump[1] && mediump[1] <= maxValue[1] && minValue[2] <= mediump[2] && mediump[2] <= maxValue[2])
	{
...
bool Star::Find_path(Node3D * node)
{
	...
	if ( node != NULL  && endp.getWalkable() && (*node == endp))
	{
		...
		path.push_back( adjNode->point );
		FILE *fp; 
		if ((fp = fopen(outputfile2_3DAStar.c_str(), "a"))!= NULL) //全局变量5
		{
